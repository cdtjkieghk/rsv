...
class RasterClassifier:
    ...
    def __init__(self, config: Config):
        ...
        try:
            ...
            self.logger.info(f"Attempting to create output directory: {self.config.output_dir}")
    ...
    def _extract_pixel_values(self, px: int, py: int) -> Optional[List[float]]:
        ...
        Returns:
            List of pixel values or None if extraction fails or contains invalid values
        """
        sample = []
        ...
        try:
            ...
            if np.any(np.isnan(sample_array)) or np.any(np.isinf(sample_array)) or np.any(np.abs(sample_array) > np.finfo(np.float32).max):
                self.logger.warning(f"Invalid values (NaN, inf, or too large) at ({px}, {py}): {sample}")
                return None
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray) -> Tuple[RandomForestClassifier, LabelEncoder]:
        ...
        # Use only classes present in y_test to avoid mismatch
        unique_labels = np.unique(y_test_encoded)
        target_names = [encoder.classes_[i] for i in unique_labels]
        self.logger.info(f"\n{classification_report(y_test_encoded, y_pred, labels=unique_labels, target_names=target_names)}")
            ...
            with rasterio.open(output_path, 'w', **meta) as dst:
                ...
                for window in tqdm(chunk_generator, total=total_chunks, desc="Processing chunks"):
                    try:
                        ...
                        stack = np.concatenate(chunk_data, axis=0)
                        h, w = stack.shape[1:]
                        reshaped = stack.reshape(stack.shape[0], -1).T

                        valid_mask = ~np.any(np.isnan(reshaped) | np.isinf(reshaped), axis=1)
                        pred = np.full(reshaped.shape[0], 0, dtype='uint8')
